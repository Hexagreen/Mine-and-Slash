...
import com.robertx22.mine_and_slash.database.spells.spell_classes.physical.finishers.EmpoweringFinisher;
...
public class PowerFormRushSynergy extends OnSpellCastSynergy {
    ...
    public BaseSpell getRequiredAbility() {
        return PowerFormSpell.getInstance();
    }
    ...
    public void tryActivate(SpellCastContext ctx) {
        ...
        EntityFinder.start(ctx.caster, LivingEntity.class, ctx.caster.getPositionVector())
            ...
            .build()
            .forEach(x -> PotionEffectUtils.apply(EmpowerEffect.INSTANCE, ctx.caster, x));
    }
